...
usage() {
    ...
    echo "  --port[port=PORT]         Set the port for the web user interface."
    ...
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --port[port=3000]"
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --port[port=3000] --data[folder=./ollama-data]"
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --port[port=3000] --data[folder=./ollama-data] --build"
    echo ""
...
echo -e "   ${GREEN}${BOLD}App Port:${NC} $app_port"
