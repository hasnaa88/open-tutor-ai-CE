describe('Support Creation Test', () => {
	...
	it('should create a new support session', () => {
		...
		cy.get('#supportTitle').should('be.visible').type('Understanding Linear Equations');
		...
		cy.contains("I'm preparing for an exam").should('be.visible').click();
		...
		cy.contains('High school').should('be.visible').click();
