	...
	$: understandActions = [
		{
			...
			icon: '🔁',
			prompt: $i18n.t('Explain this concept using a creative analogy from everyday life to make the abstract ideas more concrete and relatable.')
		},
		{
			label: $i18n.t('Example'),
			icon: '💡',
			prompt: $i18n.t('Give me a concrete, real-world application of this concept. Show me how it works in a practical scenario I might actually encounter.')
		},
		{
			label: $i18n.t('Visual'),
			icon: '📊',
			prompt: $i18n.t('Organize the key components of this topic into a clear Markdown table or a structured list to help me visualize the relationships between ideas.')
		}
	];
...
<div class="shortcut-row">
	{#if currentView === 'main'}
		...
		<button type="button" on:click={() => sendAction($i18n.t('Synthesize our conversation into 3-5 high-impact bullet points that capture the "must-know" essentials of this topic.'))} class="nav-button">
			<span>📝</span> {$i18n.t('Summarize')}
		</button>

		<button type="button" on:click={() => sendAction($i18n.t('Based on what we just covered, what is the most logical next concept I should learn? Briefly introduce it and show me how it connects to this.'))} class="nav-button">
	...
	{:else if currentView === 'understand'}
		...
		{#each understandActions as action}
			<button type="button" on:click={() => sendAction(action.prompt)} class="nav-button pulse-animation">
