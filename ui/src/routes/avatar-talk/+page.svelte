<script lang="ts">
	...
	onMount(() => {
		...
		currentMessage =
			'Welcome to the classroom! I am your AI tutor. What would you like to learn about today?';
	});
		...
		try {
				...
				messages: [
					{
						role: 'system',
						content:
							'You are an AI tutor in a classroom setting. Provide clear, educational responses.'
					},
					{
						role: 'user',
						content: userInput
			...
			if (response && 'ok' in response && response.ok) {
				...
				if (responseData.choices && responseData.choices[0] && responseData.choices[0].message) {
					...
					currentMessage =
						'I received your question but encountered an unexpected response format.';
				}
			} else {
...
<div class="h-full flex flex-col">
	<div class="flex justify-between items-center p-4 bg-gray-800 text-white">
		...
		<div class="flex gap-4">
			<button class="px-4 py-2 bg-blue-600 rounded hover:bg-blue-700" on:click={toggleClassroom}>
	...
	{#if useClassroom}
		<div class="p-2 bg-gray-700 text-white text-sm text-center">
			You're experiencing a virtual classroom with your AI tutor standing at the board. The view is
			from a student sitting in the front-row desk.
		</div>
	{/if}
</div>
